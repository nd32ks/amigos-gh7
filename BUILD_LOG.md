...
- 2026-07-17 — OWNER DIRECTIVE (supersedes stop-flag mock): role-select is now generic ("Are you the elder, guardian, or social worker?" — names removed) and every role routes to a role-scoped login. Accounts are role-bound: role written at creation, sign-in verifies users/{uid}.role, mismatch → signOut + localized error. Session gate restored (signed-in → straight to role home). Offline role check: allow + warn. Ship report: agents/V3/reports/SHIP_2026-07-17_02_role_bound_auth.md.
- 2026-07-17 — VISION NAVIGATION LAW (owner-pasted): no unified menu; role-locked surfaces. Removed SideMenu from all surfaces (files retained, unreferenced) and the companion back chevron — elder surface is now zero-navigation. Care-role permission filter on shared dashboard (diary/delegations/profile-setup hidden; trend/adherence/alerts visible). Login role chips (external layout edit) wired: role_label strings added, selected state + subtitle sync. Ship: agents/V3/reports/SHIP_2026-07-17_03_navigation_law.md.
